<template>
  <div class="list-archive">
    ...
    <div v-else-if="purchases.length === 0" class="info">Keine Einkäufe</div>
    ...
    <div v-else>
      <div
        v-for="(group, listName) in groupedPurchases"
        :key="listName"
        class="list-group"
      >
...
export default {
  ...
  methods: {
    ...
    async getData(id) {
      try {
        const response1 = await api.get(
          `/einkaufsarchiv/nutzer_gesamt/${this.user.id}`,
        );

        const response2 = await api.get(`/nutzer/${this.user.id}/listen`);
        ...
        this.errorMessage =
          error.response?.data?.detail || "Fehler beim Laden der Daten";
      } finally {
    ...
    stringLimitLength(string, comboBox) {
      ...
      if (window.innerWidth <= 480) {
        ...
        // Desktop
        if (comboBox) {
          // nur in der Combobox stark begrenzen
          maxLength = 35;
        } else {
          maxLength = 100;
        }
  ...
  computed: {
    ...
    filteredPurchases() {
      return this.purchases.filter((p) => {
        const matchesList = this.selectedListID
          ? Number(p.listen_id) === Number(this.selectedListID)
          : true;

        const matchesUser = this.onlyOwn
          ? Number(p.eingekauft_von) === Number(this.user.id)
          : true;
  ...
  mounted() {
    ...
    if (querySelectedListID !== undefined && querySelectedListID !== null) {
      this.selectedListID =
        querySelectedListID === "null" ? null : Number(querySelectedListID);
    }
