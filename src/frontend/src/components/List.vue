<template>
  ...
  <div v-if="productNum === 0" class="info">Liste leer</div>
...
export default {
  ...
  methods: {
    ...
    async add_product() {
      ...
      try {
        ...
        const responseCheck = await api.get(
          `/produkte/by-name/${encodeURIComponent(this.new_product.trim())}`,
        );
      ...
      try {
        await api.post(
          `/listen/${list_id}/produkte/${produkt_Id}/nutzer/${user_id}`,
        );
        this.errorMessage = "";
      } catch (error) {
        if (error.response) {
      ...
      if (favFound) {
        try {
          await api.put(
            `/listen/${list_id}/produkte/${favID}/nutzer/${user_id}`,
            favData,
          );
        } catch (error) {
          this.errorMessage =
    ...
    async mitglied_entfernen(mitglied_id) {
      ...
      if (
        requester_id === this.list_creator_id &&
        mitglied_id === requester_id
      ) {
    ...
    async mitglied_hinzufuegen() {
      ...
      try {
        const response = await api.post(
          `/listen/${list_id}/mitglieder/${new_member_id}`,
        );
    ...
    async delete_list() {
      ...
      try {
        ...
        const list_id = this.list_id || this.$route.params.id;
        await api.delete(`/listen/${list_id}`);
