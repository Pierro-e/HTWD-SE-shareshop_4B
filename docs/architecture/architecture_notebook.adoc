...
- Trennung von Frontend (Vue.js) und Backend (Spring Boot)
...
- Team besitzt Kenntnisse in Java, daher ist Spring Boot für das Backend sinnvoll
- Vue.js ist ebenfalls beim Großteil des Teams bekannt, was die Fronted-Entwicklung erleichtert
...
- Datenbank (PostgreSQL oder MySQL) wird als Speicher für Nutzerdaten, Listen und Finanzinformationen genutzt
...
- Frontend wird mit Vue.js (Vite) entwickelt
- Backend wird mit Spring Boot entwickelt, um REST-API zur Verfügung zu stellen
- Datenerhaltung erfolgt über eine relationale Datenbank (PostgreSQL oder MySQL)
...
- Spring Boot Backend (REST-API) läuft auf einem Server
- PostgreSQL oder MySQL, persistent
...
- Spring Boot Backend stellt eine REST-API zur Verfügung, die von der Webanwendung aufgerufen wird, um Anfragen zu bearbeiten
- Datenbank wird vom Backend angesprochen, um Daten zu speichern und abzurufen
