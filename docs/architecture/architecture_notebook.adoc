...
- die Anwendung wird als Webanwendung bereitgestellt (im späteren Verlauf wird sie um PWA-Funktionalität erweitert)
- Frontend wird mit Vue.js (Vite) entwickelt und läuft auf einem Nginx-Webserver auf der VM um einen simplen Aufruf per Mobilegerät zu ermöglichen
- mittels Vue.js werden die Komponenten der Benutzeroberfläche klar strukturiert und modular aufgebaut, mit dem Ziel einen übersichtlichen und wartbaren Code zu haben
- das Frontend ist eine Single Page Application (SPA), die im Browser des Nutzers läuft
...
- Python und MySQL wurden gewählt, da sie über SQL-Alchemy gut zusammenarbeiten
...
image::images/C4-Modell_Level_1.png[width=60%,alt="C4-Modell Level 1"]
...
image::images/C4-Modell_Level_2.png[width=60%,alt="C4-Modell Level 2"]
