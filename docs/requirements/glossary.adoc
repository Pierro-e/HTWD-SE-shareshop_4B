...
| `id` INT, `email` VARCHAR(255), `name` VARCHAR(150), `passwort_hash` VARCHAR(255)
...
| `id` INT, `name` VARCHAR(150), `ersteller` INT, `datum` DATE
| * `id` PK, eindeutig  
...
| `listen_id` INT, `nutzer_id` INT, `beigetreten_am` DATE
...
| `listen_id` INT, `produkt_id` INT, `produkt_menge` DECIMAL(10,2), `einheit_id` INT, `hinzugefugt_von` INT, `beschreibung` TEXT
...
| `id` INT, `listen_id` INT, `getatigt_von` INT, `getatigt_am` DATE, `gesamtpreis` DECIMAL(10,2)
| * `id` PK  
* `listen_id` FK → `Listen.id`  
...
| `einkauf_id` INT, `produkt_id` INT, `produkt_menge` DECIMAL(10,2), `einheit_id` INT, `produkt_preis` DECIMAL(10,2), `hinzugefugt_von` INT
...
| `id` INT, `name` VARCHAR(20), `abkurzung` VARCHAR(10)
| * `id` PK  
* `name` unique, not null
* `abkurzung` not null
| Units
